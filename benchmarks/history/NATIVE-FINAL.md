...
all exploratory transport results. The [cache-only release trial](cache-only-results.json)
missed the target at 4.604 s and is preserved. The older
[script comparison](SCRIPT-RESULTS.md) is also retained.
...
[native-final-results.json](native-final-results.json) records every command, timing, executable hash,
